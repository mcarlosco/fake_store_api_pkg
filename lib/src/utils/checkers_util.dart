...
int checkIntOrDefault(dynamic source, [int df = 0]) =>
    source is int ? source : parseIntOrDefault(source.toString(), df);
...
double checkDoubleOrDefault(dynamic source, [double df = .0]) =>
    source is double ? source : parseDoubleOrDefault(source.toString(), df);
